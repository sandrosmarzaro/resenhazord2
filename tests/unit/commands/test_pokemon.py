...
class TestPokemon:
    MOCK_POKEMON: ClassVar[dict] = _make_pokemon()

    MOCK_POKEMON_NO_ARTWORK: ClassVar[dict] = {
        **_make_pokemon(),
        'name': 'missingno',
        'sprites': {
...
class TestSinglePokemon:
    ...
    async def test_fallback_to_front_default_when_no_artwork(
        ...
        data = GroupCommandDataFactory.build(text=',pokémon')
        pokemon_route.mock(
            return_value=httpx.Response(200, json=TestPokemon.MOCK_POKEMON_NO_ARTWORK)
        )
        await command.run(data)
