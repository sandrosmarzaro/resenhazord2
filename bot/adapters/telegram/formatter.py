...
_BULLET_PATTERN: Final[re.Pattern[str]] = re.compile(r'(^|\n)- (?=\S)')
_BULLET_REPLACEMENT: Final[str] = '\\1\u2022 '
...
def whatsapp_to_html(text: str) -> str:
    ...
    with_bullets = _BULLET_PATTERN.sub(_BULLET_REPLACEMENT, escaped)
    with_blockquotes = _apply_blockquotes(with_bullets)
