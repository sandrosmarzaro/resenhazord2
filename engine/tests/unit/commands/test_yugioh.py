...
class TestSingleCard:
    ...
    async def test_returns_card_image(self, command):
        ...
        with patch('bot.domain.commands.yugioh.HttpClient.get', return_value=resp):
    ...
    @pytest.mark.anyio
    async def test_strips_newlines_from_description(self, command):
        data = GroupCommandDataFactory.build(text=', ygo')
        resp = make_json_response(MOCK_CARD_DATA)
    ...
    @pytest.mark.anyio
    async def test_caption_contains_card_name(self, command):
        data = GroupCommandDataFactory.build(text=', ygo')
        resp = make_json_response(MOCK_CARD_DATA)
...
class TestBooster:
    ...
    async def test_returns_image_buffer(self, command):
        ...
        with (
            patch('bot.domain.commands.yugioh.HttpClient.get', return_value=api_resp),
            patch(
    ...
    @pytest.mark.anyio
    async def test_booster_view_once_true_by_default(self, command):
        data = GroupCommandDataFactory.build(text=', ygo booster')
        api_resp = make_json_response(MOCK_CARD_DATA)
    ...
    @pytest.mark.anyio
    async def test_booster_show_flag_disables_view_once(self, command):
